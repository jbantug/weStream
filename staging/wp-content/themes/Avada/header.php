...
<head>
	...
	jQuery(document).ready(function($) {
		...
		$('.side-nav li a').live('click', function(e) {
			if($(this).find('.arrow').length >= 1) {
				if($(this).parent().find('> .children').length >= 1 && !$(this).parent().find('> .children').is(':visible')) {
					$(this).parent().find('> .children').stop(true, true).slideDown('slow');
				} else {
					$(this).parent().find('> .children').stop(true, true).slideUp('slow');
				}
			}
        ...
        if(retina) {
        	$('#header .logo img').attr('src', '<?php echo $data["logo_retina"]; ?>');
        	$('#header .logo img').attr('width', '<?php echo $data["retina_logo_width"]; ?>');
        	$('#header .logo img').attr('height', '<?php echo $data["retina_logo_height"]; ?>');
        }
        <?php endif; ?>
	});
	</script>
	...
	<?php if(get_post_meta($post->ID, 'pyre_page_title_bar_bg_color', true)): ?>
	.page-title-container{
		background-color:<?php echo get_post_meta($post->ID, 'pyre_page_title_bar_bg_color', true); ?>;
	}
	...
	@font-face {
		...
		src:
			...
			<?php endif; ?>
			url('<?php echo $data['custom_font_eot']; ?>?#iefix') format('embedded-opentype'),
	...
	<?php if(is_single() && get_post_meta($post->ID, 'pyre_fimg_width', true)): ?>
	#post-<?php echo $post->ID; ?> .post-slideshow,
	#post-<?php echo $post->ID; ?> .post-slideshow img{width:<?php echo get_post_meta($post->ID, 'pyre_fimg_width', true); ?> !important;}
	<?php endif; ?>

	<?php if(is_single() && get_post_meta($post->ID, 'pyre_fimg_height', true)): ?>
	#post-<?php echo $post->ID; ?> .post-slideshow, #post-<?php echo $post->ID; ?> .post-slideshow img{height:<?php echo get_post_meta($post->ID, 'pyre_fimg_height', true); ?> !important;}
	<?php endif; ?>
	...
	jQuery(document).ready(function($) {
		...
		geocoder.geocode({'address':'<?php echo $data['gmap_address']; ?>'}, function(results, status) {
			if(status == google.maps.GeocoderStatus.OK) {
				...
				$('#gmap').gmap().bind('init', function(ev, map) {
					$('#gmap').gmap('addMarker', {'position': latitude+','+longitude, 'bounds': true}).click(function() {
						$('#gmap').gmap('openInfoWindow', {'content': '<?php echo ucwords($data['gmap_address']); ?>'}, this);
					});
					$('#gmap').gmap('option', 'zoom', <?php echo $data['map_zoom_level']; ?>);
				});
			} 
		});
	});
