	...
	class WooThemes_Plugin_Updater {
		...
		function register_nav_menu_link () {	
			...
			if ( function_exists( 'add_submenu_page' ) ) {
				//$this->admin_screen = add_submenu_page( 'index.php', __( 'WooThemes Updates', 'wooslider' ), __( 'WooThemes Updates', 'wooslider' ), 'switch_themes', $this->plugin_token, array( &$this, 'admin_screen' ) );	
			}
...
<div class="wrap">
	...
	<h2><?php _e( 'WooThemes Plugin Updater', 'wooslider' ); ?></h2>
	...
	<form name="woo-plugin-updater-login" id="woo-plugin-updater-login" action="<?php echo admin_url( 'index.php?page=' . $this->plugin_token ); ?>" method="post">
		<fieldset>
			<table class="form-table">
				<tbody>
					<tr>
						<th scope="row"><label for="username"><?php _e( 'WooThemes Username', 'wooslider' ); ?>:</label></th>
						<td><input type="text" class="input-text input-woo_user regular-text" name="username" id="woo_user" value="" /></td>
					</tr>
					<tr>
						<th scope="row"><label for="password"><?php _e( 'WooThemes Password', 'wooslider' ); ?>:</label></th>
		...
		<fieldset>
			<p class="submit">
				<button type="submit" name="woo_login" id="woo_login" class="button-primary"><?php _e( 'Login', 'wooslider' ); ?></button>
			</p>
		...
		function admin_notice () {
			...
			if ( isset( $_GET['page'] ) && ( $_GET['page'] == $this->plugin_token ) ) {
				...
				if ( $this->username != '' && $this->password != '' ) {	
					...
					$notice .= '<p><strong>' . __( 'Switch WooThemes Account', 'wooslider' ) . '</strong></p><p>' . "\n";
					
					$notice .= sprintf( __( 'You are currently logged in as %1$s. To switch to a different WooThemes account, please fill in the login details below.', 'wooslider' ), '<strong>' . $this->username . '</strong>' );
					$notice .= "\n" . '</p>' . "\n";
					
					$notice .= '<p><a href="' . admin_url( 'update-core.php' ) . '">' . __( 'Update your Plugins', 'wooslider' ) . ' &rarr;</a></p>' . "\n";
				...
				if ( isset( $_GET['type'] ) && ( $_GET['type'] == 'error' ) ) {	
					...
					$notice .= '<p><strong>' . __( 'Login Error', 'wooslider' ) . '</strong></p><p>' . "\n";
					
					$notice .= __( 'The login details supplied are invalid. Please try again.', 'wooslider' );
					$notice .= "\n" . '</p>' . "\n";
			...
			if ( ( ! isset( $_GET['page'] ) || ( isset( $_GET['page'] ) && ( $_GET['page'] != $this->plugin_token ) ) ) && ( $this->username == '' || $this->password == '' ) && ( get_option( $this->plugin_prefix . 'hide-admin-notice', false ) != true ) ) {
				$notice = '<div id="woo-plugin-updater-notice" class="updated fade">' . "\n";
				$notice .= '<p class="alignleft"><strong>' . __( 'Enable WooThemes Plugin Updates.', 'wooslider' ) . '</strong> ' . "\n";
				
				$notice .= sprintf( __( 'Please <a href="%1$s">login</a> to enable automatic plugin updates.', 'wooslider' ), 'index.php?page=' . $this->plugin_token );
				$notice .= "\n" . '</p>' . "\n";
				...
				$notice .= '<p class="alignright submitbox"><a href="' . $url . '" class="submitdelete">' . __( 'Hide This Message', 'wooslider' ) . '</a></p>' . "\n";
