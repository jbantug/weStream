...
add_shortcode('content_box', 'shortcode_content_box');
	function shortcode_content_box($atts, $content = null) {
		...
		$str .= '<span class="more"><a href="'.$atts['link'].'">'.$atts['linktext'].'</a></span>';
		endif;
...
function shortcode_recent_posts($atts, $content = null) {
	...
	if($atts['excerpt'] == "yes"):
	$stripped_content = string_limit_words(get_the_content(), $atts['excerpt_words']);
	$html .= '<p>'.strip_shortcodes($stripped_content).' [...]</p>';
	endif;
	$html .= '</article>';
	$count++;
	endwhile;
					...
					$html .= '<li>';
						$html .= '<div class="image">';
								...
								$html .= '<div class="image-extras">';
									...
									$html .= '<a class="icon" href="'.$full_image[0].'" rel="prettyPhoto[gallery<?php echo get_the_ID(); ?>]"><img src="'.get_bloginfo('template_directory').'/images/finder-ico.png" alt="'.get_the_title().'" /></a>';
...
function shortcode_social_links($atts, $content = null) {
	...
	foreach($atts as $key => $link) {
		if($link) {
			$html .= '<li class="'.$key.'">
			<a class="'.$key.'" href="'.$link.'">'.ucwords($key).'</a>
			<div class="popup">
				<div class="holder">
					<p>'.ucwords($key).'</p>
				</div>
...
function shortcode_client($atts, $content = null) {
	$html = '<li>';
	$html .= '<a href="'.$atts['link'].'"><img src="'.$atts['image'].'" alt="" /></a>';
	$html .= '</li>';
	return $html;
}
